...
const EditMovementForm = ({setCurrentMovement, movement, setMovements}) => {
  ...
  <div className='fixed top-0 left-0 w-screen h-screen flex items-center justify-center bg-black bg-opacity-50'>
    <form className='bg-white p-8 rounded-lg flex flex-col gap-4 w-3/4 md:w-1/2 relative' action={handleSubmit}>
      <button type='button' className='absolute top-4 right-4 text-xl text-black' onClick={() => setCurrentMovement(null)}><X /></button>
      <h2 className='uppercase text-black font-bold'>Edit Movement</h2>
      <label className='text-black' htmlFor='name'>Name: </label>
      <input className='text-black bg-white border' type="text" name="name" defaultValue={movement.name} />
      <label className='text-black' htmlFor='link'>Link: </label>
      <input className='text-black bg-white border' type="text" name="link" defaultValue={movement.link} />
