...
export function SigninForm() {
  ...
  const [state, formAction, pending] = useActionState(signinUser)
  ...
  return (
    <form action={formAction} className="space-y-6">
      <div className="space-y-4 border-2 border-gray-200 p-4 rounded-md">
        ...
        <button type='submit' className='w-full border-2 py-2 rounded-md bg-black text-white hover:bg-gray-500'>{pending ? "Submitting" : "Let me in"}</button>
