...
import { verifySessionForRequests, verifySession } from './session';
...
const findUserByName = async (input) => {
  try {
    await connectDB();
    ...
    if (user.role !== "admin") {
      throw new CustomError("You do not have permission to search users", 403)
    }
...
const getPaginatedUsers = async ({ page = 1, limit = 10 }) => {
  try {
    ...
    // if user is not admin, throw error
    if (user.role !== "admin") {
      throw new CustomError("You do not have permission to get paginated users", 403)
    }

    if (!page || !limit) {
      throw new CustomError("Invalid page or limit", 400)
    }
...
const deleteUser = async (userId) => {
  try {
    ...
    // if user is not admin, throw error
    if (user.role !== "admin") {
      throw new CustomError("You do not have permission to delete users", 403)
    }
