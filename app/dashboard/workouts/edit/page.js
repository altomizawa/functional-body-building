...
export default function EditWorkoutForm() {
  ...
  const handleMovement = (e) => {
    ...
    setFilteredMovements(
      movements.filter(movement =>
        movement.name.includes(e.target.value.toLowerCase())
      )
    )
  }
  ...
  const removeMovement = (movement) => {
    setSection(prevSection => ({
      ...prevSection,
      movements: prevSection.movements.filter(
        (prevMovement) => prevMovement.name !== movement.name
      )
    }))
  }
  ...
  return (
    <form onSubmit={onSubmit} className='h-full px-6 my-16 max-w-[1440px] mx-auto'>
      ...
      <div>
        ...
        <div id='sections' className='my-4 backdrop:border-2 space-y-24'>
          {/* SECTION */}
          {newWorkout.sections && newWorkout.sections.map((section, index) => (
            <div className='space-y-8 border-2 p-6' key={index}>
            ...
            <div className='w-full space-y-2'>
              ...
              <div className='relative'>
                {filteredMovements && filteredMovements.length > 0 && (
                  <ul className='absolute top-4 left-0 bg-white border-[1px] border-black/20 shadow-md'>
                    {filteredMovements?.map((movement, index) => (
                      <div
                        key={index}
                        className='px-2 py-2 flex items-center gap-2 cursor-pointer hover:bg-black hover:text-white'
          ...
          <button
            type='button'
            className='button__main-menu my-2 uppercase'
            onClick={addNewSection}
          >
            {newWorkout.sections.length === index+1 ? 'add section' : 'remove section'}
          </button>
          </div>))}
        </div>
      </div>
